...
def parallel_block_sort(arr, block_size):
    n = len(arr)
    ...
    with mp.Pool() as pool:
        blocks = pool.map(block_merge_sort, blocks)

    # Merge the sorted blocks sequentially
    while len(blocks) > 1:
        next_blocks = []
...
if __name__ == "__main__":
    ...
    print_memory_usage()
    block_size = 5
    IntData = parallel_block_sort(IntData, block_size)
    print("parallel_block_sort")
    print_memory_usage()
